...
class MultisigFinish extends Command {
  ...
  async run () {
    try {
      const { flags } = this.parse(MultisigFinish)
  ...
  async finishTx (flags, sigs) {
    try {
      ...
      const tx = new bitcore.Transaction(txObj)
  ...
  validateFlags (flags) {
    ...
    if (!tx || tx === '') {
      throw new Error('You must specify an unsigned multisig transaction with the -t flag.')
    }
...
MultisigFinish.description = `Retrieve signatures, sign multisig TX, and broadcast
...
MultisigFinish.flags = {
  name: flags.string({ char: 'n', description: 'Name of wallet' }),
  tx: flags.string({ char: 't', description: 'Unsigned transaction in JSON format' }),
  txids: flags.string({ char: 'a', description: 'Array of TXIDs of messages containing signatures' })
}

module.exports = MultisigFinish
